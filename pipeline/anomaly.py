...
def run():
    ...
    df["iqr_freight_flag"] = iqr_flag(df["freight_value"].fillna(0))
    ...
    df["is_anomaly"] = df["iqr_price_flag"] | df["iqr_freight_flag"] | df["iso_anomaly"]

    anomalies = df[df["is_anomaly"]][[
        "order_id", "order_date", "price", "freight_value", "revenue",
        "customer_state", "product_category_name_english",
        "iqr_price_flag", "iqr_freight_flag", "iso_anomaly",
    ]]

    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    anomalies.to_csv(OUTPUT_PATH, index=False)
    print(f"  Anomalies detected: {len(anomalies):,} / {len(df):,} rows ({len(anomalies)/len(df)*100:.1f}%)")
